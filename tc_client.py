...
def c(row,col):
    ...
    if rowcol in foxes:
        ...
        r1.append([row,col])
    print(len(r1))
    TerroristTargetedCntLabel=Label(root,width=20,text=str(len(r1)) + " Terror Cells Targeted ")
    TerroristTargetedCntLabel.grid(row=9,column=n+4)
    TerroristRemainingCntLabel=Label(root,width=20,text=str(Foxes-len(r1)) + " Terror Cells Remaining ")
    TerroristRemainingCntLabel.grid(row=11,column=n+4)
    
    
  
def show_terrorists(connection):
    global hits
    hits=0
    ...
    msg="hits:"+str(hits)+",misses:"+str(Foxes-hits)+",drone count:"+str(droneCnt)
    send(connection,msg)
...
def testRun(event,testRow,testCol):
    ...
    while 1:
        ...
        if  (p[0] >-1 and p[0] < n and p[1] < n and p[1] > -1  ):
...
def receive(client, addr):
    ...
    while True:
        ...
        dc_msg=msg.decode(FORMAT)
        msg_list=dc_msg.split(";")
        if msg_list[0]=="INITIATE":
            gr2=msg_list[1]
        print ("In receive")
        print(gr2)
        print ("Out receive")
        sys.stdout.flush()
        if msg_list[0] == "SCORE":
            # if connection.getsockname()==c_score.split(":")[1]:
                   # print(connection.getsockname() + " is me")
            scoreLabel=Label(root,text="Scores:")
            scoreLabel.grid(row=scoreRow,column=n+4) 
            for item in range(1,len(msg_list)):
               print(msg_list[item])
               score=msg_list[item].split(":")
               #for s in range(0,len(score)):
                # print(s)
                # print(score[s])
               if score[0]==str(connection.getsockname()):
                  scoreLabel=Label(root,fg="blue",text="Player " + str(item) + " Score : " + str(score[1]))
                  scoreRow=scoreRow +1
                  scoreLabel.grid(row=scoreRow,column=n+4) 
               else:
                  scoreLabel=Label(root,text="Player " + str(item) + " Score : " + str(score[1]))
                  scoreRow=scoreRow +1
                  scoreLabel.grid(row=scoreRow,column=n+4)  
               
               print("*"+score[0]+ "*")
               print("*"+str(connection.getsockname())+"*")
               #print(score[1])
               if score[0]==str(connection.getsockname()):
                   print ("This is me")
...
b = Button(root, height= 1, width=10,text="Fire",bg="red",fg="black",command=lambda: show_terrorists(connection))
