...
function MissionFlyby({ viewerRef }) {
  ...
  useEffect(() => {
    ...
    // Flight path: east → west at low altitude through the AO
    const FLY_LAT   = 34.920;  // tuned to land at vertical screen centre
    const LON_START = 71.280;  // well off right edge
    const LON_END   = 70.640;  // well off left edge
    ...
    const N_S = 12;
    let terrH = new Array(N_S).fill(850);
    ...
    C.sampleTerrainMostDetailed(viewer.terrainProvider, terrPts)
      .then(pts => { terrH = pts.map(p => p.height ?? 850); })
      .catch(() => {});
...
const CENTER_LAT = 34 + 46/60 + 20.39/3600;
const CENTER_LON = 70 + 57/60 +  0.68/3600;

// Hard-coded strike targets — MOAB must land within `radius` metres for a kill
const TARGETS = [
  { id: 1, lat: 34.772545, lon: 70.952621, radius: 4.57 }, // ~15 ft — target 1
  { id: 2, lat: 34.763585, lon: 71.000826, radius: 4.57 }, // ~15 ft — target 2
  { id: 3, lat: 34.734586, lon: 70.979386, radius: 4.57 }, // ~15 ft — target 3
];
const INIT_ALT   = 5184;   // 3600 × 1.2 × 1.2 — 40 % zoomed out
...
function placeTankAt(viewer, lat, lon, tp, addLog) {
  if (!viewer || viewer.isDestroyed() || !window.Cesium) return;
  const C = window.Cesium;
  const GUN_D  = 0.000055; // ~6 m north — gun barrel tip (true scale)
  const REAR_D = 0.000040; // ~4 m south — rear hull
  const SIDE_D = 0.000025; // ~3 m E/W  — roll gradient
  const pts = [
  ...
  C.sampleTerrainMostDetailed(tp, pts)
    .then(([cPt, gunPt, rearPt, ePt, wPt]) => {
      ...
      const cH = cPt.height ?? 800, gunH = gunPt.height ?? cH;
...
function detonateStrike(viewer, lat, lon, groundAlt, addLog, onHit) {
  ...
  // Replace with a static entity once fully expanded — stops callbacks running forever
  setTimeout(() => {
    if (viewer.isDestroyed()) return;
  ...
  const SW_SPEED   = 600;   // m/s — supersonic shockwave
  const SW_MAX_R   = 8000;  // m — sweeps well beyond any visible area
  const soundDelay = Math.min(Math.max(horizDist / SW_SPEED, 1.5), 8.0) * 1000; // ms

  // Acceleration: speed increases 20% by the time wave reaches SW_MAX_R
  // v(t) = SW_SPEED + SW_ACC*t, so r(t) = SW_SPEED*t + 0.5*SW_ACC*t²
  const SW_ACC      = (SW_SPEED * 0.2) / (SW_MAX_R / SW_SPEED); // ~9 m/s²
  ...
  // Blast sound + screen concussion flash when wave arrives at camera POV
  setTimeout(() => {
    playBlastSound();
  ...
  // Reusable Color objects — never allocate inside callbacks
  ...
  const getColor = () => {
    const fadeIn  = Math.min(1, elapsedS / 0.5);                                    // 0→1 over first 0.5 s
  ...
  // Stop all callbacks and remove model at 35 s (scorch stays permanently)
  setTimeout(() => {
...
const BRIEF_IMAGES = [
  { src: '/vent.png',   label: 'VENTILATION SHAFT — ALPHA' },
  { src: '/vent_2.png', label: 'VENTILATION SHAFT — BRAVO' },
...
const NARRATION_SCRIPT =
  ...
  'These images show the shaft openings. Study them. ' +
...
function Level3Briefing({ onReady }) {
  ...
  // Start narration on mount — uses same female voice as levels 1 & 2
  useEffect(() => {
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.62rem', color: 'var(--t-dim)', lineHeight: 2, letterSpacing: '.04em' }}>
            THREE VENTILATION SHAFTS HAVE BEEN IDENTIFIED ABOVE THE UNDERGROUND FACILITY.
...
// Self-contained countdown — only this component re-renders every second
...
export default function Level3Game({ playerName, onPlayAgain, onMissionComplete }) {
  ...
  const [pendingTank,    setPendingTank]    = useState(null);   // { lat, lon }
  const [placedTankList,      setPlacedTankList]      = useState([]);
  const [missionAccomplished, setMissionAccomplished] = useState(false);
  const [failReason,          setFailReason]          = useState(null); // null = not failed
  const [showFlyby,           setShowFlyby]           = useState(false);
  const [briefing,            setBriefing]            = useState(true);
  const [log,      setLog]      = useState([
    { ts: timestamp(), msg: 'LEVEL 3 — DIRECT ACTION — NANGARHAR PROVINCE', cls: 'warn' },
  ...
  useEffect(() => {
    ...
    const HALF_M = 6.096; // half of 40 ft in metres
    TARGETS.forEach(t => {
      ...
      viewer.entities.add({
        polygon: {
          ...
          material: new C.ColorMaterialProperty(C.Color.fromCssColorString('#8B3A2A').withAlpha(0.92)),
          classificationType: C.ClassificationType.TERRAIN,
        },
      });
    });
    ...
    // Left-click: tank placement or strike target
    ...
    handler.setInputAction((click) => {
      ...
      const ep = { position: pos, heightReference: C.HeightReference.CLAMP_TO_GROUND,
                   disableDepthTestDistance: Number.POSITIVE_INFINITY };
  ...
  const handleLaunchStrike = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      detonateStrike(viewerRef.current, target.lat, target.lon, target.groundAlt || 0, addLog, (sLat, sLon) => {
        ...
        let hitTarget = null;
        TARGETS.forEach(t => {
          if (destroyedRef.current.has(t.id)) return; // already destroyed
  ...
  const handleTestFlyby = useCallback(() => {
    setShowFlyby(false);
    setTimeout(() => {
      triggerFlyby(viewerRef.current, () => setShowFlyby(true));
    }, 50);
  }, []);
    ...
    <div className="game-wrap">
      ...
      <div className="hud">
        <div className="hud-brand">
          TACT/CMD
          <span>LEVEL 3 — DIRECT ACTION</span>
        </div>
        ...
        <div className="sidebar">
          ...
          <div className="panel">
            ...
            <div className="intel-row">
              ...
              <span className="intel-val" style={{ fontSize: '.6rem' }}>34°46′N 70°57′E</span>
            </div>
            <div className="intel-row">
            ...
            <div className="intel-row">
              ...
              <span className="intel-val">Nangarhar</span>
            </div>
            <div className="intel-row">
